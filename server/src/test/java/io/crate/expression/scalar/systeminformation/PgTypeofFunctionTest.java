...
public class PgTypeofFunctionTest extends ScalarTestCase {
    ...
    public void test_primitive_types() {
        ...
        assertEvaluate("pg_typeof(58::byte)", DataTypes.BYTE.getName(), Literal.of(DataTypes.BYTE, (byte) 58));
    ...
    public void test_subscript_expression_types_and_errors() {
        ...
        assertThatThrownBy(() -> assertNormalize("pg_typeof({x=1}::object(STRICT)['y'])", isLiteral("")))
            .hasMessageContaining("The cast of `{x = 1}` to return type `OBJECT(STRICT)` does not contain the key `y`");
        ...
        assertThatThrownBy(() -> assertNormalize("pg_typeof({x=1}::object(DYNAMIC)['y'])", isLiteral("")))
            .hasMessageContaining("The return type `OBJECT(DYNAMIC) AS (\"x\" INTEGER)` of the expression `{x = 1}` does not contain the key `y`");
