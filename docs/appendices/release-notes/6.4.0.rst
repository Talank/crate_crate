...
- Changed the PostgreSQL type mapping of the ``byte`` type from ``"char"``
  ...
  ``int2`` in both text and binary format. This affects clients that use
  user-created columns of the ``"char"`` type (not recommended); CrateDB
  doesn't use ``"char"`` internally.
