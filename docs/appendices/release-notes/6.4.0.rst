...
- Changed the PostgreSQL type mapping of the ``byte`` type from ``"char"``
  (oid 18) to ``int2`` (oid 21), as ``byte`` is a numeric type while
  PostgreSQL's ``"char"`` is a single-character string type. Clients now
  receive ``byte`` columns as ``int2`` in both text and binary format.
